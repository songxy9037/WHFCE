...
class NewLoss(nn.Module):
    ...
    def forward(self, predict, gt):
        ...
        loss_cross_entropy = cross_entropy(predict, gt)
        loss2 = self.dice_coef(gt, predict)
        loss = loss_cross_entropy + 0.5 * loss2
        return loss

    def dice_coef(self, gt, predict):
        intersection = torch.sum(gt * predict)
        union = torch.sum(gt) + torch.sum(predict)
