...
def show_feature_map(feature_map,
                     epoch):  # feature_map=torch.Size([1, 64, 55, 55]),feature_map[0].shape=torch.Size([64, 55, 55])
    ...
    for index in range(1, feature_map_num + 1):  # 通过遍历的方式，将64个通道的tensor拿出
        ...
        # 将上行代码替换成，可显示彩色
        plt.imshow(transforms.ToPILImage()(feature_map[index - 1].cpu().numpy()),
                   cmap='jet')  # feature_map[0].shape=torch.Size([55, 55])
        plt.axis('off')
        # scipy.misc.imsave('feature_map_save//' + str(index) + ".png", feature_map[index - 1])
        plt.imsave('feature_map/' + str(index + epoch) + ".png", feature_map[index - 1].cpu().numpy(), cmap='jet')
        # plt.savefig(r'X:\LSK\ChangeFormer-main\feature_map\t1.jpg')
